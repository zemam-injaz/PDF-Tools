...
class EnhancedPDFAnalyzer:
    ...
    def export_annotations_to_markdown(self, file_path: str) -> str:
        """Export annotations to markdown format (skips pages with no annotations)"""
        try:
            doc = fitz.open(file_path)
            ...
            for page_num in range(len(doc)):
                ...
                # Collect annotations with content for this page
                page_annotations = []
                if annotations:
                    for annot in annotations:
                        annot_content = annot.info.get("content", "")
                        if annot_content:
                            page_annotations.append((annot.type[1], annot_content))

                # Only add page header if there are annotations with content
                if page_annotations:
                    has_any_annotations = True
                    content += f"## Page {page_num + 1}\n\n"

                    for annot_type, annot_content in page_annotations:
                        content += f"- **{annot_type}:** {annot_content}\n"

                    content += "\n"

            doc.close()
            return content if has_any_annotations else ""
