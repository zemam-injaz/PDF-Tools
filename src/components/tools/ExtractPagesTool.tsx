...
import { FileOutput, LayoutGrid, ArrowRight, CheckCircle, AlertCircle, Sparkles } from 'lucide-react';
...
export const ExtractPagesTool: React.FC = () => {
  ...
  const handleInputChange = (path: string) => {
    setInputPath(path);
    if (!outputPath && path) {
      setOutputPath(getDefaultOutputPath(path, '_extracted'));
    }
  };
  ...
  const handleOperation = async () => {
    ...
    const res = await api.extractPages(inputPath, outputPath, pages);
    
    setLoading(false);
    
    if (res.success) {
      setStatus({ type: 'success', message: 'تم استخراج الصفحات بنجاح!' });
      setInputPath('');
      setOutputPath('');
      setPages('');
    } else {
    ...
    <div className="card h-full flex flex-col animate-fade-in relative z-0">
      {showVisualSelector && (
        <VisualPageSelector 
          ...
          title="تحديد الصفحات للاستخراج"
          confirmLabel="تأكيد صفحات الاستخراج"
        />
      )}

      <div className="flex items-center gap-4 mb-8 pb-6 border-b border-border">
        <div className="p-4 rounded-xl bg-green-50 text-green-600 shadow-sm">
          <FileOutput size={28} />
        </div>
        <div>
          <h2 className="text-2xl font-bold text-primary">استخراج صفحات</h2>
          <p className="text-secondary mt-1">استخراج صفحات معينة من ملف PDF في ملف جديد.</p>
        </div>
      </div>
      ...
      <div className="flex-1 flex flex-col gap-8 overflow-y-auto pr-1 custom-scrollbar">
        <section className="space-y-6">
          ...
          <div>
            <div className="flex items-center justify-between mb-2.5">
              <label className="block text-sm font-semibold text-primary">2. تحديد الصفحات للاستخراج</label>
              {inputPath && (
                <button 
                  onClick={() => setShowVisualSelector(true)}
                  className="text-green-600 hover:text-green-700 flex items-center gap-2 text-sm font-bold px-3 py-1.5 bg-green-50 hover:bg-green-100 rounded-lg transition-colors border border-green-100"
                >
            ...
            <input
              ...
              className="input border-green-100 focus:border-green-500 focus:ring-green-500"
            />
        ...
        <section className="mt-auto pt-4 pb-2">
          ...
          <button
            ...
            className="btn-enhanced w-full py-4 text-lg shadow-lg transition-all disabled:opacity-50 disabled:shadow-none text-white bg-green-600 hover:bg-green-700 shadow-green-500/20 hover:shadow-green-500/30"
          >
