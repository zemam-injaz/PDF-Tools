...
export const TextExtractTool: React.FC = () => {
  ...
  const handleExtract = async () => {
    ...
    // Generate output path - use outputDir if provided, otherwise same location as input
    let finalOutputDir = outputDir;
    if (!finalOutputDir) {
      // Try to extract directory from input path
    ...
    // Use custom filename if provided, otherwise derive from input
    ...
    const res = await api.extractTextToFile(
      inputPath,
      outputPath,
      pageRange || 'all',
      format,
      true, // Always merge lines
    ...
    <div className="card h-full flex flex-col animate-fade-in">
      ...
      <div className="flex-1 flex flex-col gap-8 overflow-y-auto pr-1 custom-scrollbar">
        <section className="space-y-6">
          ...
          <p className="text-xs text-secondary -mt-4 mr-1">سيتم حفظ الملف في: {outputDir || 'نفس المجلد'} / {customFilename || '(الاسم الأصلي)'}.{format}</p>
        </section>
