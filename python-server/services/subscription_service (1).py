...
class SubscriptionService:
    ...
    def _get_features(self, plan: PlanType, status: SubscriptionStatus) -> list[str]:
        if status != SubscriptionStatus.ACTIVE:
            return ["basic_reading"] # Expired = Free features
        
        if plan == PlanType.FREE:
            return ["basic_reading"]
        elif plan == PlanType.TRIAL:
            return ["basic_reading", "fast_reading", "pro_stats", "sync"]
        else:
             return ["basic_reading", "fast_reading", "pro_stats", "sync", "priority_support"]
